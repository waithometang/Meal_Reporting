...
<template>
  <div class="OrderFood" :class="{'action': logoIsmboa}">
    <HreadStatusBar :GMBC_Show_title="!logoIsmboa" :GMBC_type="GMBC_type" @GMBC_advance="advance" @GMBC_current="current" @GMBC_retreat="retreat"></HreadStatusBar>
    <div class="FoodList">
      <div class="content">
        ...
        <div class="WeekMeals" :class="{'StaleDated': items.isOver}" v-for="(items, indexs) in iniData" :key="indexs">
          ...
          <div class="meals">
            <ul>
              <li v-for="(item,index) in items.baocan" :key="index" :class="{'action-li': typeindexs == indexs && typeindex==index && showLi}" @click="predetermine(item,items, index,indexs)" @touchstart='touchstart_Li(index, indexs, item, true)' @touchend='touchend_Li(false)'>
                <div v-if="item.schedule_id" class="title" :class="{'Centered_title': item.qczt === 0, 'StaleDated-color': items.isOver}">{{mealsData[index]}}</div>
                <div v-if="item.schedule_id" class="time" :class="{'notpayment': (item.qczt === 1 || item.qczt ==3 || item.qczt ==4 || item.qczt ==5) && !item.isOver, 'acton-color': item.qczt === 2 && !item.isOver,'StaleDated-color': item.isOver}">{{getState(item)}}</div>
              </li>
            </ul>
          </div>
        </div>
    ...
    <mt-popup :isShow='isShow' :ShowData='ShowData' @ChangeIsshow="ChangesState" @RefreshData="_GetMyReport"></mt-popup>
  </div>
</template>
