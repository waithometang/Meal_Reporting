    ...
    <div class="item">
      <ul class="main-ul" v-for="(item, index) in listData" :key="index">
        ...
        <li class="mian-li-left">
          <span class="title-font"><img src="../imgaes/zaocan.png"/>{{item.mealName}} &nbsp;&nbsp;¥ {{item.price}} 元</span>
        ...
        <li class="main-li-right_1" v-if="item.takeStatus===1 || item.takeStatus===2">
          <div class="font"  @click="showQrCode(item)">
            <div class="btu color_1">
              {{item.takeStatus===1 ? '取餐' : '等待开餐'}}
            </div>
          </div>
          <div class="font"  @click="gotoshow(item)">
            <div class="btu color_2">
              分享
            </div>
          </div>
        </li>
        ...
        <li class="main-li-right" v-else-if="item.takeStatus===3">
...
<script>
import { MyTakeMeal, GetToAll } from '@/api/baocan'
